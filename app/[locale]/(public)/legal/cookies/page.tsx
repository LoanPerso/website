...
export default function CookiesPage() {
  ...
  useEffect(() => {
    const loadContent = async () => {
      try {
        const response = await fetch(`/content/legal/${locale}/cookies.md`);
        if (response.ok) {
          const text = await response.text();
          setContent(text);
        } else {
          const fallback = await fetch(`/content/legal/fr/cookies.md`);
          if (fallback.ok) {
            setContent(await fallback.text());
          }
        }
      } catch (error) {
        console.error("Failed to load cookies content:", error);
      } finally {
        setIsLoading(false);
      }
    };
            ...
            <div ref={containerRef} className="flex-1 min-w-0">
              ...
              <nav className="flex flex-wrap gap-1.5 mb-6 pb-4 border-b border-foreground/10 md:gap-2 md:mb-8 md:pb-6">
                {LEGAL_PAGES.map((page) => {
                  ...
                  const isActive = page.key === "cookies";
                  return (
                    <Link
                      key={page.key}
