...
export default function LegalNoticesPage() {
  ...
  useEffect(() => {
    const loadContent = async () => {
      try {
        const response = await fetch(`/content/legal/${locale}/notices.md`);
        if (response.ok) {
          const text = await response.text();
          setContent(text);
        } else {
          const fallback = await fetch(`/content/legal/fr/notices.md`);
          if (fallback.ok) {
            setContent(await fallback.text());
          }
        }
      } catch (error) {
        console.error("Failed to load notices content:", error);
      } finally {
        setIsLoading(false);
      }
    };
            ...
            <div ref={containerRef} className="flex-1 min-w-0">
              ...
              <nav className="flex flex-wrap gap-1.5 mb-6 pb-4 border-b border-foreground/10 md:gap-2 md:mb-8 md:pb-6">
                {LEGAL_PAGES.map((page) => {
                  ...
                  const isActive = page.key === "legalNotices";
                  return (
                    <Link
                      key={page.key}
                      ...
                      h1: ({ children }) => {
                        ...
                        return (
                          <h1
                            id={id}
                            data-section-id={id}
                            className="font-serif text-2xl mb-3 leading-tight text-foreground scroll-mt-24 sm:text-3xl md:text-4xl md:mb-4 md:scroll-mt-28"
                          >
                            {children}
                          </h1>
                        );
