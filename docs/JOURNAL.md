...
- **Scroll natif :** `overscroll-contain` + `admin-scroll` sur les **4** zones scrollables (liste, lecteur, dossiers, corps du composer) — plus de scroll-chaining/rebond.
- **Anti-débordement :** adresses De/À/Cc et corps du message en `break-words` + `min-w-0` (fin du scroll horizontal sur longues adresses/URL).
- **Feel natif :** `active:` (retour de pression) sur tous les boutons custom ; `select-none` ; **`touch-action: manipulation` posé globalement** sur les contrôles `.admin-theme` (`globals.css`) — fin du délai de tap. Composer : Brouillon/Envoyer pleine largeur au pouce (`flex-1 sm:flex-initial`) ; titre de la barre mobile renforcé (`text-[15px] font-semibold`).
- **Inchangé & sûr :** flux single-pane mobile (dossiers → liste → lecteur) + `lg:contents` desktop intacts ; **aucune logique** modifiée (uniquement classes Tailwind + 1 règle CSS globale). Inputs déjà ≥16px sur mobile (anti-zoom iOS, géré dans `globals.css`). `tsc --noEmit` **vert**.
- **Périmètre :** webmail (inbox/lecteur/composer + barre comptes). Le sandbox `/admin/mail/simulate` et la page `accounts` (formulaire) n'ont pas été retouchés.
- **Docs :** CHARTE_GRAPHIQUE (Ergonomie tactile + `mail/*`).

## 2026-05-30 — Prod : migration des données business depuis la préprod (hors users & mails)
- **Brief :** copier en **prod** toutes les données métier de la préprod **sauf** les users (`admin_users`) et la messagerie (`mail_*`).
- **Prérequis schéma :** la prod était en retard de **5 migrations** (2026-05-30) — `ledger_entries` absente, vues P&L/`v_stats_*`/`v_arrears_summary`, fonctions `rpc_stats_*`/cache KPI, et le fix `set_reference` (encore tronquant). Appliquées d'abord via `db-apply.mjs` ciblé prod (`SUPABASE_PROJECT_REF=$SUPABASE_PROD_PROJECT_REF`), idempotentes. Prod désormais à **14 migrations**.
- **Analyse préalable (lecture seule) :** prod business **vide** (0) ; **produits aux UUID identiques** préprod/prod → aucun remapping de `product_id` ; **toutes les FK `auth.users` nulles** en préprod (`created_by`/`assigned_to`/`computed_by`…) → aucun user requis sur la cible ; tables CRM annexes (`contracts`, `client_scores`, `client_documents`, `interactions`, `tasks`, `import_batches`, `activity_log`) **vides** → rien à copier.
- **Migration :** nouveau script `scripts/migrate-preprod-to-prod.mjs` (service role, lecture paginée 1 000 / insertion chunkée, **ordre FK**, **UUID & références préservés**, garde-fous : refus si source = cible ou prod non vide hors `--force`, URL cible vérifiée contre le ref prod). 6 tables copiées : **clients 1 367, loan_applications 220, loans 1 987, installments 47 688, payments 23 779, ledger_entries 38**.
- **Post :** séquences `client/loan/payment_ref_seq` réalignées au max migré (**1367/1987/23779**) pour éviter les collisions futures ; `refresh_portfolio_kpis()` recalculé (`kpis_cache` non copié, reconstruit).
- **Vérifié :** `src=dst` sur les 6 tables ; **0 orphelin FK** (produit/client/loan/installment/payment/application) ; **KPIs & P&L prod = préprod à l'identique** — encours **591 372 €**, décaissé 1 065 506 €, encaissé 556 662 €, intérêts 82 527 €, défaut 2,42 % ; CA **109 238 €**, bénéfice affiché **70 624 € / 64,7 %**, bad debts **40 174 €**, bénéfice éco. 30 450 € / 27,9 %. Exclusions intactes : `admin_users` 1, `products` 8, `mail_accounts` 7, `mail_messages` 44.
- **Nature des données — IMPORTANT :** c'est le **jeu smoke calibré (fictif, généré)**, pas de vraies données client → **wiper avant toute exploitation réelle** de la prod.
- **Non commité** (attente d'autorisation). **Docs :** DATABASE, PROJECT_STATUS, DECISIONS.

## 2026-05-30 — Statistiques : refonte UI (FilterBar + suppression header/KPI) + couche RPC paramétrée
- **Brief :** retravailler les 8 onglets « Statistiques » — retirer titre/description (`PageHeader`) et les cartes KPI, ajouter une **barre de filtre ultra-complète** (plage de dates + dimensions) en tête, et réalimenter chaque onglet avec des stats plus riches et performantes.
- **Archi data (migration `20260530190000_analytics_rpc.sql`) :** les vues `v_stats_*` (sans paramètre) ne pouvaient pas être filtrées par date → remplacées par **8 fonctions RPC `rpc_stats_*`** (`portfolio`, `products`, `risk`, `vintages`, `collections`, `cashflow`, `clients`, `funnel`). Chacune renvoie **un seul json** couvrant tous les graphes de la page, agrégé server-side sous un **filtre commun** : fenêtre de dates (origination / période / création / demande) + dimensions (`product`, `country`, `risk`, `status`, `source`). `security invoker` (RLS admin), `grant execute` aux `authenticated`. Choix imposé par l'échelle : **1 987 prêts > plafond PostgREST 1 000 lignes**, l'agrégation doit rester serveur (un fetch brut client tronquerait silencieusement).
- **FilterBar (`app/_components/admin/filter-bar.tsx`) :** rail réutilisable, inline, grayscale, **sans modal** — présets (30 j / 90 j / 6 m / 12 m / YTD / Tout) + range custom (deux `input[type=date]`), selects de dimensions contextuels, chips de filtres actifs + reset. Exporte les `*_OPTIONS` alignés sur le catalogue + `defaultStatsFilters()`.
- **Kit partagé (`app/_components/admin/stats-kit.tsx`) :** `useStatsData(fetcher, filters)` (refetch debouncé piloté par les filtres, ignore les réponses périmées), `StatGrid` (tuiles `Metric` denses, remplacent les `KpiCard`), `MiniBars` (barres horizontales classées), `StatsLoading`/`StatsError`.
- **8 pages réécrites :** plus de `PageHeader` ni de `KpiCard` ; `FilterBar` en tête, sous-onglets `Tabs`, contenu réalimenté. Nouvelles stats : origination mensuelle, distributions montant/durée, splits statut/produit/pays, taux de défaut par tranche de montant, exposition par produit, perte par cohorte, arriéré par produit/risque, encaissements par méthode de paiement, nouveaux clients par mois, conversion par tranche de score/montant, etc.
- **Méthode :** fondation écrite + testée à la main (migration appliquée préprod, 8 RPC vérifiées sans/avec filtres, page `portfolio` exemplaire) puis **fan-out de 7 agents** (un par page) contre le contrat figé, revue + typecheck.
- **Vérifié :** migration appliquée à la **préprod** (jamais la prod), les 8 RPC renvoient des json cohérents (sans filtre + filtrés date/dimension), `grant execute` OK, cache PostgREST rechargé. Couche TS typée 1:1 sur les payloads. **`tsc --noEmit` vert** sur tout le projet. Aucun import interdit (`PageHeader`/`KpiCard`/modal) dans `stats/`. **Non commité** (attente d'autorisation).
- **Docs :** DATABASE, CHARTE_GRAPHIQUE, PROJECT_STATUS.

## 2026-05-30 — Centre de Statistiques : groupe sidebar + 8 pages spécialisées + 18 vues analytics
- **Sidebar :** nouveau groupe **« Statistiques »** (8 entrées) sous le Tableau de bord — Portfolio, Risque & scoring, Recouvrement, Cashflow, Cohortes, Produits, Clients, Origination. Chaque page a des **sous-onglets** (`Tabs`).
- **Vues analytics (migration `20260530180000_analytics_views.sql`) :** 18 vues `v_stats_*` (agrégations server-side, `security_invoker`, admin only) : portfolio overview (avg/médiane/p90, taux/durée moyens), tranches de montant, durées, par produit, par pays ; risque par catégorie (exposition, défaut, perte), bandes de score ; DPD buckets + dunning ; cashflow réalisé+projeté ; vintages (cohortes par mois d'origination, % remboursé/défaut) ; clients (overview, statut, revenu, top exposition) ; funnel (statut, overview/conversion, sources).
- **Générateur enrichi :** `generate-smoke.mjs` assigne désormais `product_id` (par tranche de montant) + `credit_score` (dérivé du risque) aux clients, et génère **220 demandes** (`loan_applications`, statuts/sources/scores variés, ~38 converties). Re-wipe + rechargement préprod.
- **Couche & UI :** `app/_lib/admin/analytics.ts` (1 fonction/vue, typée), types `Stat*` dans `types.ts`, kit de graphes **déjà existant réutilisé** (`LineChart`/`DonutChart`/`GroupedBars`/`StackedBar` + `Bar`) — aucune dépendance ni composant nouveau, aucun modal. Pages sous `app/admin/(protected)/stats/<thème>/page.tsx`.
- **Cohérence brief (vues live) :** exposition moyenne **432,61 €** (brief 432,72), médiane **357 €** (~350), p90 **847 €** (~850) ; produits : micro-crédit 561 / avance 566 / conso 121 / étudiant 119 ; risque A 621 / B 338 / C 265 / D 143 ; funnel 220 demandes, conversion 17,3 %.
- **Vérifié :** les 18 vues renvoient des données cohérentes ; **8 pages** présentes ; `tsc --noEmit` **vert**. Migration appliquée à la préprod. **Non commité** (attente d'autorisation).
- **Sidebar :** la nav débordait (8 entrées en plus) → scrollbar par défaut moche. Ajout d'une utilitaire `.admin-scroll` (scrollbar fine 6px, piste transparente, pouce `--border` se renforçant au survol — Firefox + WebKit) dans `globals.css`, appliquée à la zone scrollable de `admin-shell.tsx`. Scoppée à l'admin (le site public n'est pas touché).
- **Docs :** DATABASE, DATA_ARCHITECTURE, PROJECT_STATUS.

## 2026-05-30 — Préprod : régénération smoke calibrée (1 367 clients) + module Finances/P&L + correctifs d'échelle
- **Objectif :** valider que l'outil tient « en usage » à volume réel et reproduire la masse du brief consolidé 2024-2025 (suivi Excel → DB). Cible **préprod uniquement** (jamais la prod).
- **Module Finances/P&L (migration `20260530170000_finance_pnl.sql`) :** table `ledger_entries` (revenus/dépenses manuels que le portefeuille ne déduit pas : coaching, server fees, management loans, rebranding) + vues `v_pnl_monthly`/`v_pnl_summary` qui **fusionnent le dérivé** (intérêts/frais/pénalités depuis loans+installments) **et le manuel** (ledger) + bad debts (write-offs). RLS admin, normalisation `period_month` au 1er du mois.
- **Échelle (migration `20260530171000_perf_scale.sql`) :** index composites (`installments(status,due_date)`, `installments(loan_id,status)`, `payments(status,payment_date)`, `tasks/client_documents/contracts(client_id,status)`), table `kpis_cache` + fonctions `refresh_portfolio_kpis()`/`get_portfolio_kpis()` (security definer), vue `v_arrears_summary`.
- **Correctifs côté code :** `dashboard.getKpis` lit le cache (`rpc get_portfolio_kpis`, fallback vue) ; `clients.listClientOverview` en `count:estimated` ; `collections.listArrears` + `installments.listOverdue` **paginés** (.range+count) ; `collections.arrearsSummary` (KPI globaux via `v_arrears_summary`) ; page **Recouvrement** : contrôles de pagination + KPI alimentés par le résumé. Nouvelle page **`/admin/finance`** (P&L, structure des revenus, écritures en **panneau inline**, pas de modal) + entrée nav « Finances (P&L) ».
- **Bug latent corrigé (migration `20260530172000_fix_reference_seq.sql`) :** `set_reference()` utilisait `lpad(n::text,4,'0')` qui **tronque** au-delà de 9999 (sémantique Postgres) → collision de référence (`PAY-10000`→`'1000'`). Cassait tout insert d'entité > 9 999 lignes (clients/loans/payments/contracts) en prod. Révélé par les 23 717 paiements smoke. Fix : padding ≥ 4 sans troncature.
- **Générateur `scripts/generate-smoke.mjs` :** wipe préprod (Management API, garde `admin_users`+`products`, reset séquences) puis génération calibrée — **1 367 clients actifs**, distribution par tranches, 24 mois, 12,5 %, mix ≈ 80 % à jour / 13 % retard / 7 % douteux (dont 48 défauts/write-offs), + 620 prêts historiques soldés 2024 (historique mensuel + intérêts). Mise à l'échelle des principaux pour caler encours = 591 529 € et bad debts = 40 328 €. Garde anti-prod, flag `--wipe`/`--dry`.
- **Résultat vérifié (vues live) :** 1 367 clients / 1 987 prêts / **47 688 échéances** / 23 717 paiements. Encours **591 204 €** (cible 591 529), total prêté **1 068 234 €**, intérêts **82 918 €**, défaut 2,42 %, impayés 21 637 € / 274 dossiers. **P&L : CA 109 655 € (brief 109 815), dépenses 38 614 € (exact), bénéfice affiché 71 042 € / marge 64,8 % (brief 71 201 / 64,8 %), bad debts 40 001 €, bénéfice éco. 31 040 € / 28,3 % (brief 30 873 / 28,1 %).**
- **Échelle mesurée :** `v_portfolio_kpis` brute **1 724 ms** → cache **91 ms** (~19×) ; `v_client_overview`/recouvrement/impayés **95-152 ms** paginés ; vues P&L ~85 ms. `ANALYZE` post-chargement (count estimé exact = 1 367). **Conclusion : l'outil tient à 1 367 clients** une fois ces correctifs en place.
- **Non modélisé (assumé) :** le P&L consolidé vit désormais via `ledger_entries` + dérivés ; pas de plan comptable complet. Les write-offs restent comptés dans l'encours par la vue existante (non modifiée) tout en étant reconnus en perte au P&L — voir DECISIONS.
- **Typecheck `tsc --noEmit` vert.** Migrations appliquées à la préprod ; **non commité** (en attente d'autorisation).
- **Docs :** DATABASE, DATA_ARCHITECTURE, PROJECT_STATUS, DECISIONS.

## 2026-05-30 — Messagerie : comptes business + 27 conversations smoke en prod
- **6 comptes ajoutés** (+ `contact@quickfund.ee` complété) → **7 comptes** business multi-domaines : `support`/`recouvrement`/`comptabilite@quickfund.fr`, `commercial`/`partenariats@quickfund.eu`, `direction`/`contact@quickfund.ee`. Chacun avec **config IMAP/SMTP smoke** (993/ssl, 587/starttls, `*_password` write-only). États variés réalistes : `comptabilite` en **erreur SMTP**, `partenariats` en **IMAP non testé**.
- **Dossiers :** le `createAccount` de l'UI ne crée **pas** les dossiers → `contact@` en prod avait **0 dossier**. Création des **4 standards** pour tous (+ **Archive/Indésirables** pour `contact`/`support`) → **32 dossiers**.
- **27 conversations smoke** (**39 messages**, **6 pièces jointes**, **11 diagnostics**) : scénarios crédit réalistes (info conso, KYC, remboursement anticipé, virement SEPA, RGPD, relance/délai recouvrement, leasing/LOA, regroupement, micro-crédit, partenariats/courtage, factures compta, phishing en Indésirables, archive). Threading `thread_key`/`in_reply_to`, non-lus/suivis, in/out.
- **Seed versionné idempotent :** `supabase/seed_mailbox.sql` (comptes `on conflict`, dossiers `unique(account_id,path)`, messages gardés par `thread_key like 'smk-%'`). Appliqué **à la prod uniquement** via l'API Management (**HTTP 201**).
- **Vérifié :** 7 comptes / 32 dossiers / 39 messages / **27 threads** / 6 PJ / 11 diagnostics ; **0 `folder_id` orphelin** ; 1 sortant sans `in_reply_to` (relance ouvrant un fil — attendu) ; spam→phishing, archive→remerciements OK. Données **déjà live** (prod déployée pointe cette DB).
- **Décision :** inverse la clause « pas de mails démo en prod » du 2026-05-29 (messagerie = mockup) ; vrais enregistrements métier toujours vides.
- **Docs :** DATABASE, DECISIONS, PROJECT_STATUS.

## 2026-05-30 — Déploiement : CI Netlify réparé (cassé depuis ~2 mois) → prod en ligne
- **Symptôme :** tous les déploiements `main` échouaient depuis le 2026-04-03 (dernier succès 2026-02-10) — étape `npx netlify deploy --prod` : `Error: Unauthorized: could not retrieve project`. **Pas un bug de code** (build vert). Le site live servait encore le build du 10 février (`/admin/login` → 404 en prod alors qu'il existe dans le code).
- **Cause :** secret GitHub `NETLIFY_AUTH_TOKEN` (modifié le 2026-03-10, pile avant le 1er échec) **invalide** ; le déploiement passe par **GitHub Actions** (`.github/workflows/netlify-deploy.yml` → CLI `netlify deploy`), pas par l'intégration Git native Netlify (site `repo: None`).
- **Correctif :** nouveau token Netlify fourni → validé via l'API (`/user` = `lynxerprv@gmail.com`, HTTP 200). Site cible identifié par l'API : `candid-starship-f7ee02` = `quickfund.fr` = **`9cd6597f-5cbb-4d37-883e-eaea821a1afb`**. Les **2 secrets GitHub** mis à jour (`NETLIFY_AUTH_TOKEN`, `NETLIFY_SITE_ID`) via `gh secret set` (valeurs jamais affichées) ; token + site id aussi rangés dans `.env`.
- **Relance + suivi :** `gh run rerun` du run en échec (même commit `2a18b3f`, event=push) → **`deploy-production` success en 8m50s**.
- **Vérifié en live :** `quickfund.fr/admin/login` **404 → 200** ; deploy Netlify publié `2026-05-30T11:11:30Z` (`ready`) ; check-run GitHub `deploy-production: success`.
- **DB ciblée :** env Netlify déjà sur la **prod** — `NEXT_PUBLIC_SUPABASE_URL` = `…aqwenqsxdubyhhjkfekh…`, clé anon = celle de **prod** (empreinte vérifiée, pas de mismatch). `NEXT_PUBLIC_SITE_URL` absent → fallback code `https://quickfund.fr` (correct).
- **Note sécurité :** le token a transité en clair dans le chat — **à rotationner** quand possible (régénérer côté Netlify + re-`gh secret set`).
- **Docs :** CLAUDE.md §12.2 (2 clés Netlify), `.env.example` (bloc Netlify CI), PROJECT_STATUS.
